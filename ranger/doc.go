...
A [*Ranger] ought to be constructed with [New] and any options required by your application.
Calling [New] - without any options - is valid and fully operational.
...
[DefaultLogger] provides a sane [logger.Logger] intended to cover most use cases,
...
Constructing a [*Ranger] with [New] reads from the OS's environment variables.
...
and then pass that into [New].
Or, more conveniently, configure the Ranger's Router after [New]
and all the way up until calling [*Ranger.Guide].
As a last resort, overwriting the *Ranger.Router initially configured
