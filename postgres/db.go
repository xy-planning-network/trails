...
func (db *DB) Begin(opts ...*sql.TxOptions) *DB {
	// TODO(dlk): decide on error handling, rollback func
...
func (db *DB) Create(value any) error {
	...
	isPointer := reflect.TypeOf(value).Kind() == reflect.Pointer
	isSlice := reflect.TypeOf(value).Kind() == reflect.Slice
	if !isPointer && !isSlice {
		return fmt.Errorf("%w: %T must be a pointer or slice", trails.ErrUnaddressable, value)
	}
...
// Delete archives the database record.
...
func (db *DB) Exists() (bool, error) {
	var exists bool
	subQ := db.db
	err := db.db.Debug().Raw("SELECT EXISTS(?)", subQ).Scan(&exists).Error
	if err != nil {
		err = fmt.Errorf("%w: %s", trails.ErrUnexpected, err)
		return false, err
	}
...
// Paged turns the results of the query into a paginated version: PagedData.
func (db *DB) Paged(page, perPage int64) (PagedData, error) {
	...
	err := db.db.Session(new(gorm.Session)).Count(&totalRecords).Error
	if err != nil {
		err = fmt.Errorf("%w: %s", trails.ErrUnexpected, err)
		return pd, err
	}
...
// TODO(dlk): docstring
func (db *DB) Where(query any, args ...any) *DB { return &DB{db.db.Where(query, args...)} }
...
//
// If not records are updated, ErrNotFound returns.
